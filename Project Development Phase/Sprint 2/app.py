...
def calcstreak(userid,curdate):
    ...
    try:
        query="select date,streak from userexpenses where userid='"+userid+"' and date='"+prevdate+"'"
        streakss=ibm_db.execute(conn,query)
        streak=ibm_db.fetch_assoc(streakss)
        while streak:
            streaks=int(streak[1])
            streak=ibm_db.fetch_assoc(streakss)
        streaks+=1
    except:
        streaks=1
...
def myexpense():
    ...
    return render_template("expenses/expense.html",userecons=userecons)
...
def myincome():
    ...
    return render_template("incomes/income.html",userecons=userecons)
